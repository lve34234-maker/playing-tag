...
| --- | --- |
| 이동 | 방향키 또는 `W` `A` `S` `D` |
| 달리기 | `Shift` (스태미나 소모, 멈추면 회복) |
| 웅크리기 | `C` (5초간 투명 · 쿨타임 15초 · 움직이면 발자국이 남음) |
| 잡기 (술래일 때) | `Space` 또는 화면의 `CATCH` 버튼 |

모바일/터치 기기에서는 화면 좌측의 방향 패드와 우측의 `RUN` / `웅크리기` / `CATCH` 버튼을 사용합니다.

## 👑 관리자 모드

닉네임 칸에 **`어드민140531`** (또는 `admin140531`)을 입력하면 관리자로 입장합니다(화면 표시 이름은 **이현석**).
관리자는 필드에 참여하지 않고 **전체 맵을 관전**하며 패널로 게임을 제어합니다.

- **플레이어 목록** — 접속자, 술래/정지/웅크림/팀 상태 확인
- **전체 제어** — 전원 집결 · 전원 흩뿌리기(랜덤 순간이동) · 전원 동결/해제 · 팀전 ON/OFF · 리셋
- **미니게임**
  - 🏃 **술래잡기** — 기본 모드
  - ☄️ **운석 피하기** — 떨어지는 운석을 피하는 생존전(맞으면 탈락)
  - 🟥🟦 **판 뒤집기** — 두 팀이 바닥 타일을 자기 색으로 칠해 더 많이 차지한 팀이 승리
